...
截至 0.2.0，仓库已完成指导书 T00～T11 中可在本机可靠闭环的核心部分：
...
- 控制超时或任务失败会标记执行不确定性，框架不自动重试；Python 会立即终止未知状态的 host 会话，禁止继续复用。
- `DIRECT_OPERATE_NR` 明确返回 `UNSUPPORTED_BY_BACKEND`，不会模拟成功。
- PICS 三态门禁、能力 ID 与 389 行矩阵交叉校验、`dnp3_dut`、`dnp3_unsupported_behavior`、`dnp3_state_changing` 风险门禁。
- 有界 `stats` 状态、可移植包脚本和不接真实 EMS 的本机一键自检。
...
- 当前未建立持久 unsolicited 测量收集器；启动时不自动扫描，也不自动启用/禁用 Class unsolicited。
- 没有 `capture.begin/progress/end`、PCAP 采集、黄金字节播放器、Raw frame 注入或独立一致性工具适配。
...
先读：`docs/standards/ems_device_profile.md`、`config/ems_profile.example.json`、`schemas/ems-profile.schema.json`、`python/src/dnp3_master/pytest_plugin.py`、`config/capability_matrix.csv`。

操作：

- 复制示例为 `config/ems.local.json`；填写准确设备身份和 Profile revision。
- 每项只按原始 PICS/Device Profile 填 `SUPPORTED`、`NOT_SUPPORTED` 或 `UNKNOWN`。
- 另建 `config/points.local.csv`，至少包含点类型、索引、Class、读写属性、量程、反馈点和风险等级。
...
..\.venv\Scripts\python.exe -m pytest tests\test_host_read.py --collect-only `
...
修改范围：只新增 `python/tests/dut/test_ems_read.py` 及必要的本地数据加载 fixture；不改控制代码。
...
输出至少包括：参考端产品/版本/配置、host `build-info.json`、Git commit、PICS/点表哈希、开始结束时间、测试 ID 结果、PCAP 哈希、日志哈希、环境/网卡说明和评审人。
...
停止条件：生产设备、无书面授权、无回退、点号/值不确定、CROB 点模型或 FC3/FC4 支持状态不明、联锁状态未知、操作超时或反馈不一致。超时后先人工核实状态，严禁自动重发。

### H05（P1，T12）：Unsolicited 完整时序

唯一目标：实现持久 unsolicited 收集与 Enable/Disable/Confirm 时序；不要同时实现周期扫描。

先读：固定 OpenDNP3 Master 配置、`ISOEHandler`/`IMasterApplication`、开发指导书 M3、PICS 主动上送策略和标准内部条款。

建议修改：新增独立 `OpenDnp3EventSupport` 或等价小模块，复用有界测量模型；Host 增加明确的 enable/disable/consume API；Python 增加类型化事件接口。不得让回调阻塞命令线程。

本机测试至少覆盖：空 unsolicited、事件上送、Confirm、序号回绕、重发/重复、Confirm 丢失、solicited 交错、队列溢出和重连。随后在真实 EMS 按 PICS 验证。
