...
## 当前环境补充：模拟设备（2026-09-07）

用户已确认被测 EMS 连接的是模拟设备，遥控遥调不限下发。已实现
[SIMULATOR 模式](SIMULATOR_MODE.md)，不需要内网 agent 再开发移除安全门：配置
`dnp3_simulator = true`，使用 `ems_test_plan.simulator.example.json`，即可批量/重复
控制，无需审批、DUT/operator ID、PICS 解锁、持久事故锁或强制恢复。
下文工单、人工事故确认、逐次选场景仅针对 LAB，不应重新套到模拟器。
内网仍需完成：模拟 EMS 地址/链路地址、实际点号和反馈映射、控制模型/响应、事件
预期值和接收验证、实际网络的性能和稳定性验收。模式不会生成虚构测量，也不会使
未实现的 FC6/32 位 qualifier 可用。此次没有 EMS 联调或 IEEE 一致性证据。

### 当前需求范围裁剪与新增交付

用户进一步确认：**EMS 监听，pytest 主站仅主动连接；不需要时间同步、Restart；
外部模拟器已能产生信号变化，不开发触发接口。** 因此 H06、H07a 和触发适配器不是
本次交接待办。其余通用扩展卡保留作未来候选，不是当前基本验证的阻塞。

已增加 [单配置入门套件](SIMULATOR_QUICKSTART.md)：`examples/pytest_simulator` +
`settings.local.json`，包含自动发现/版本检查、分层诊断、静态四类点、Class 0 与
...
当前模拟 EMS 基本测试按文首裁剪范围和单配置指南执行，不要求先取得下面所有通用
认证/扩展输入。角色已确定为 EMS 监听、pytest TCP Client；现场需填连接、点号、
反馈和期望，配合外部信号验证。审批/危险等级/恢复值要求仅针对 LAB。

下面是正式互操作、一致性或扩展能力验收的通用输入清单；只收集当前任务需要的部分，
缺少独立证据时不得形成最终认证结论：

1. EMS 厂商、型号、固件、Device Profile/PICS 版本及其批准来源。
2. 连接参数：EMS Outstation/TCP Server 的 IP、监听端口、本地网卡、主/从链路地址、最大分片和超时要求；当前连接角色无需重新开发。
3. 点表及场景计划输入：对象类型、索引、Class、量程、工程单位、初值、可写性、控制反馈点、操作/恢复值、危险等级和批准工单。
4. 主动上送、启动完整性和 Confirm 策略；时间同步、Restart 不在当前范围，Freeze/Assign Class 仅未来按需确认。
...
私有输入放在测试目录的 `settings.local.json`、`config/*.local.*`、`secrets/` 或
`evidence/local/`，不得提交到 GitHub。
...
2. 配置/执行验收卡优先填写私有配置并运行现成用例，不为“完成开发”而改代码。
   确需修复代码时先补失败测试再最小实现；不得用只改文档或固定假数据冒充功能修复。
...
8. 代码卡按仓库验证要求运行相应测试；协议/能力变化才同步更新 Schema、能力矩阵和
   `hello.capabilities`，不得仅因配置验收通过就升级能力状态。纯文档卡检查链接和命令，
   配置/执行卡保存实际结果；只有得到提交授权后才创建独立 commit。
9. 不确定控制结果不得自动重发，必须关闭会话。LAB 按事故手册核对和确认，禁止删除/编辑活动锁；SIMULATOR 不访问事故锁，新 client 或下一用例可继续。
...
### H06（当前不需要；未来候选，T13）：时间同步

用户已明确不需要时间同步，不作为本次内网待办；以下设计只供未来需求变更时参考。
...
H07a Restart 已由用户排除；H07b/H07c 也是按需扩展，不是当前基础套件的接入前置。
...
当前模拟 EMS：集中配置 → 连接/只读 → 控制反馈 → 外部事件 → 目标负载/24 小时。
H06/H07a 不执行，不开发外部模拟器触发接口。以下图示仅为通用完整验收/未来扩展路线：
