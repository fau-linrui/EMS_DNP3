...
适用范围：本手册中的审批、持久锁和人工确认流程仅针对 LAB 控制模式。
全部为模拟设备的环境请用 [SIMULATOR 模式](SIMULATOR_MODE.md)：无需这些流程，
不访问现存 LAB 事故锁；不确定结果仍报错并关闭当前 host，新 client 可继续。
...
`KeyboardInterrupt`（例如 Ctrl+C）和 `SystemExit` 如果发生在控制请求可能发出后，
同样进入以下流程，包括等待响应和结果校验阶段。完成事故记录与会话销毁后重新
抛出原中断，避免自动化误以为可以继续控制。发包前的输入校验中断不创建事故锁。
断电、强制结束 Python 等无法执行清理代码的情况仍需按独立人工读回流程处置。
...
`examples/pytest_ems/test_control_scenarios.py` 不会因为全局解锁而批量运行控制。私有计划中的场景必须启用，并且本次命令必须用 `--dnp3-control-scenario <精确ID>` 选择一个场景。已授权状态改变测试会拒绝 pytest-xdist，模板也会拒绝同一 pytest 进程内的 rerun/repeat。模板按以下顺序执行：基线读回、一次操作、操作后读回、一次预批准恢复、恢复读回。

只有操作结果明确成功且操作后反馈达到批准值，模板才会发送恢复命令。如果操作后读回超时、缺失或值不一致，设备状态无法确认，模板会停止且不盲目恢复；此时即使核心事故锁没有触发，也必须停止后续控制并人工读回。操作或恢复的协议结果一旦被客户端判定为不确定，则由本手册的持久事故锁流程接管。任何阶段都不会自动重发控制。
...
查询事故锁只把明确的“文件不存在”当作无锁。文件权限、读取、存储访问或格式错误
均保持 LAB 控制锁定，不使用可能吞掉访问错误的 `Path.exists()` 作为放行依据。
此规则不改变 SIMULATOR 不访问事故目录的行为。
