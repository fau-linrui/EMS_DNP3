...
| EMS 私有测试场景计划 | MISSING | 0.5.0 已提供严格模板、加载器和不连接 DUT 的四输入离线预检，但尚未填写真实完整性/Class 期望、主动上报触发、批准控制/反馈/恢复值和工单引用 | DUT_OWNER / SAFETY_OWNER | 真实 EMS 场景执行；控制和主动上报默认关闭 |
...
- T00～T12 的通用工程、TCP、核心 Read/IIN、有响应控制和显式 unsolicited 基线路径已实现；0.5.0 还提供可复制的严格 EMS pytest 场景套件、可编程有状态回环从站和离线配置预检。本机完整 native 链路与同栈集成测试通过，真实 EMS 和原始 Confirm/重发故障时序仍未验证。
