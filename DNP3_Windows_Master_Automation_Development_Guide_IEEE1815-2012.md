...
当前用户环境已确认为模拟设备，遥控遥调允许不限下发。已实现显式
[SIMULATOR 模式](docs/SIMULATOR_MODE.md)：无需 LAB 的审批、operator/DUT ID、
持久事故锁、单场景选择和强制恢复；支持批量/重复控制。本文后续相关安全门规范
仅适用于 LAB，不应成为模拟器测试的额外限制。协议校验、有界资源、失败报告和
反馈断言继续适用；模拟器运行不构成真实设备或 IEEE 一致性证据。
